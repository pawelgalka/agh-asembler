#----------------------------------------------------------
# Funkcja do programu lab_10
#
# Compute Pi (Leibniz formula) using SSE instructions
#----------------------------------------------------------

	.data
	.align 16	#the compiler will add enough padding bytes so the next data lands on an "even" position (divisible by the alignment value).
...
fun_a:
	...
	inc %rdi		# half of iterations is enough - in case iter is odd, we increment so as we don't make less than half iterations
	
	movdqa	denom, %xmm5	# denominators to xmm5
	movdqa	numer, %xmm2	# numerators to xmm2
	movdqa	add4, %xmm3	# differences to xmmhttps://www.uci.agh.edu.pl/laboratoria-uci/mops/3
...
next:
	...
	movsd	%xmm1, %xmm0	# low part to xmm0 - if function returns a value, this value must be in xmm0 register
...
fun_b:
	...
	inc %rdi		# quarter of iterations is enough - in case iter is odd, we increment so as we don't make less than quarter iterations
...
next_a:
	...
	haddps	%xmm1, %xmm1	# horizontal sums of low & high parts
	movdqa  zero, %xmm0	# clear xmm0 as two doubles - a means align
